...
describe("Exposes", () => {
  ...
  it("rolls back tunnels when domain normalization fails", async () => {
    ...
    try {
      ...
      expect(createFailProvider.destroyTunnel).toHaveBeenCalledWith(
      ...
      expect(restoreFailProvider.destroyTunnel).toHaveBeenCalledWith(
        "https://broken-restore.example.com"
      );
      await second.stop();
    } finally {
      await rm(dir, { recursive: true, force: true });
  ...
  it("applies new auth hash when update reactivates a dormant endpoint", async () => {
    ...
    try {
      ...
      expect(addRouteSpy.mock.calls[0]?.[2]).toBe(
        updated.endpoint.auth?.passwordHash
      );
...
function providerBuild(options: {
  ...
  const createTunnel = vi.fn<[number, "public" | "local-network"], Promise<{ domain: string }>>(
    async () => {
