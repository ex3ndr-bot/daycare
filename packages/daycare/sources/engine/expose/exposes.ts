...
export class Exposes implements ExposeProviderRegistrationApi {
  ...
  async create(
    ...
    return this.lock.inLock(async () => {
      ...
      const now = Date.now();
      const id = createId();
      const auth = normalizedInput.authenticated
        ? await authEntryCreate()
        : null;
  ...
  async update(
    ...
    return this.lock.inLock(async () => {
      ...
      const updated: ExposeEndpoint = {
        ...current,
        auth: auth
          ? { enabled: true, passwordHash: auth.passwordHash }
          : null,
        updatedAt: Date.now()
      };
  ...
  private async endpointsLoad(): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        logger.warn(
          { endpointId, error },
          "skip: Failed to restore expose endpoint"
        );
      }
    }
  }

  private async endpointActivate(
    endpointId: string,
  ...
  private async endpointWrite(endpoint: ExposeEndpoint): Promise<void> {
    ...
    await atomicWrite(
      this.endpointPathBuild(endpoint.id),
