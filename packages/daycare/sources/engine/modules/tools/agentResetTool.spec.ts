...
describe("agentResetToolBuild", () => {
  ...
  it("passes optional reset message", async () => {
    ...
    await tool.execute(
      { agentId: "agent-target", message: "Manual reset requested." },
      context,
      toolCall
    );
  ...
  it("throws when target agent does not exist", async () => {
    ...
    await expect(
      tool.execute({ agentId: "missing-agent" }, context, toolCall)
  ...
  it("throws when attempting to reset the current agent", async () => {
    ...
    await expect(
      tool.execute({ agentId: "agent-source" }, context, toolCall)
    ).rejects.toThrow("Cannot reset the current agent.");
