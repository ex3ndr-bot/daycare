...
describe("sandboxAllowedDomainsValidate", () => {
  it("rejects wildcard domains", () => {
    expect(sandboxAllowedDomainsValidate(["*"], true))
      .toEqual(['Wildcard "*" is not allowed in allowedDomains.']);
  });

  it("requires network permission when any domains are present", () => {
    expect(sandboxAllowedDomainsValidate(["example.com"], false))
      .toEqual(["Network permission is required to set allowedDomains."]);
  });
