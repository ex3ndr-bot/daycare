 ...
 */
export function sandboxAppsAccessCheck(
  permissions: SessionPermissions,
  target: string
): SandboxAppsAccessCheckResult {
...
function sandboxAppsRootResolve(
  permissions: SessionPermissions,
  target: string
): string | null {
  const candidates = Array.from(
    new Set(
      [permissions.workingDir, ...permissions.readDirs].map((entry) =>
        sandboxExistingPathResolve(entry)
      )
    )
  );
