...
describe("migration20260220AddUsers", () => {
  ...
  it("enforces connector_key uniqueness", () => {
    ...
    try {
      migration20260220AddUsers.up(db);
      db.prepare(
        "INSERT INTO users (id, is_owner, created_at, updated_at) VALUES (?, ?, ?, ?)"
      ).run("user-1", 1, 1, 1);
      db.prepare(
        "INSERT INTO users (id, is_owner, created_at, updated_at) VALUES (?, ?, ?, ?)"
      ).run("user-2", 0, 1, 1);
      ...
      db.prepare(
        "INSERT INTO user_connector_keys (user_id, connector_key) VALUES (?, ?)"
      ).run("user-1", "telegram:123");

      expect(() => {
        db.prepare(
          "INSERT INTO user_connector_keys (user_id, connector_key) VALUES (?, ?)"
        ).run("user-2", "telegram:123");
      }).toThrow();
    } finally {
      db.close();
    }
